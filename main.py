...
async def startup_event():
    ...
    # 从外部API同步智能体
    try:
        from core.external_agent_sync import sync_external_agents
        result = await sync_external_agents(agent_registry)
        print(f"外部智能体同步结果: {result}")
    except Exception as e:
        print(f"同步外部智能体时发生错误: {e}")
